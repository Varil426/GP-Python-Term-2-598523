...
while True:
    ...
    if chosen_option == 1:
        ...
        if destination_place is not None:
            ...
            country_name = get_country_full_name(country)
            print(
                f"Miasto {destination_place} leży w kraju {country_name}\n Długości geograficzna: {lon}, szerokość geograficzna: {lat}"
            )
        else:
        ...
        if origin_place is not None:
            if destination_place is not None:
                ...
                print(
                    f"Kurs walut 1 {origin_currency} = {currency_ratio}{destination_currency}"
                )
